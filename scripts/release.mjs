 ...
 *
 * 1. Creates the release `v<version>` up front (electron-builder races itself
 *    when two artifacts try to create the same release, ending in a 422).
...
if (!release) {
  ...
  release = await gh('/releases', {
    method: 'POST',
    body: JSON.stringify({ tag_name: tag, name: tag, body: `ghetto-blocker ${version}`, draft: false, prerelease: false }),
  });
} else {
...
const final = await gh(`/releases/tags/${tag}`);
